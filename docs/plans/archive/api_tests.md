...
| 4   | `argus/backend/controller/testrun_api.py`                 | `argus/backend/tests/testrun_api/test_testrun_api.py`             | `complete` | Added: get_type, get_testrun (+null), set_status (+missing), investigation_status (+missing), assignee set/clear/missing (notification mocked), comments (post/get/single/unknown/update/delete), activity, fetch_results, get_runs_by_test_id_run_id, ignore_jobs (+empty reason), pytest results & stats. Issues endpoints (submit/event-submit/get/get-missing-key/delete) use `mock_issue_service` (patches `IssueService` in the controller module). All 9 Jenkins routes use `mock_jenkins_service`. S3 redirects (log download, screenshot proxy) use `mock_s3`. `terminate_stuck_runs` runs against the live SCT model (returns total=0 with no seeded stuck rows). |
...
| 6   | `argus/backend/controller/api.py` (users/jobs/polls)      | `argus/backend/tests/api/test_users_jobs_api.py`                  | `complete` | 17 tests covering `/users`, `/user/token` (POST issues an additional token per call with `duration`/TTL, null = non-expiring, invalid rejected; GET reports the calling token's expiration), `/user/{jobs,planned_jobs}`, `/test_runs/poll` (recent + dedup with `additionalRuns[]`), `/test_run/poll` (success + unknown + empty), `/artifact/resolveSize` (HTTP HEAD mocked, success/error/missing-link), `/zeus/<endpoint>` error paths (missing host, missing token), `/test_run/comment/get` (unknown/missing/round-trip via comments/submit). Real S3/Zeus calls not exercised; happy-path Zeus proxy deferred until a Zeus mock fixture exists.                                                                                                                                                  |
...
| 11  | `argus/backend/controller/notification_api.py`            | `argus/backend/tests/notification_api/test_notification_api.py`   | `complete` | 12 tests covering `/api/v1/notifications/{get,get_unread,summary,read}`. Seeded via direct `ArgusNotification.save()` against `g.user.id` with per-test cleanup fixture. Validates unread counting (read excluded), summary pagination (`limit`, default 20, `afterId` clustering DESC), full vs short-summary dict shapes, and read mutation round-trip via subsequent GETs. Error paths cover missing/unknown ids on both `/get` and `/read`.                                                                                                                                                                                                                             |
| 12  | `argus/backend/controller/view_api.py`                    | `argus/backend/tests/view_api/test_view_api.py`                   | `complete` | 21 tests covering `/api/v1/views/{,/create,/get,/all,/update,/delete,/search,/<id>/{versions,images,resolve,resolve/tests,pytest/results},/stats}`. Item parsing exercised for `release:`/`group:`/`test:` prefixes; verification done via paired GET endpoints. Error paths cover missing/unknown ids and duplicate names. `/views/stats` smoke-tested for missing-viewId error, empty-view stable shape, and widget-id filter path (no SCT data required).                                                                                                                                                                                                                |
| 13  | `argus/backend/controller/team.py`                        | `argus/backend/tests/team_api/test_team_api.py`                   | `complete` | 13 tests covering `/api/v1/team/{create, /<id>/get, /<id>/edit, /<id>/motd/edit, /<id>/delete, /user/<id>/teams, /user/<id>/jobs, /leader/<id>/teams}`. Verifies leader auto-inclusion in members, ownership-guard error paths on edit/motd/delete (non-leader), unknown-id errors, and that `user/<id>/teams` lists both owned and member teams. `user/<id>/jobs` exercised via empty-result path against unassigned user. Per-test cleanup of `Team` rows.                                                                                                                                                                                                                |
| 14  | `argus/backend/controller/planner_api.py`                 | `argus/backend/tests/planner_api/test_planner_api.py`             | `complete` | 20 tests covering `/api/v1/planning/{,/plan/{create,<id>/get,update,<id>/owner/set,<id>/resolve_entities,<id>/delete,<id>/copy,<id>/copy/check}, /plan/trigger, /release/<id>/{all,gridview}, /search, /group/<id>/explode}`. Verified via paired GETs (including `/api/v1/views/get` for plan-view linkage on delete). Auto-created view per plan covered (delete+keep variants). `/plan/copy` covered via empty-tests path asserting release/version/description/participants on the new plan. `/plan/trigger` covered for no-plans, missing-filter, and empty-plan paths using `mock_jenkins_service` to assert Jenkins is not invoked.                                  |
| 15  | `argus/backend/controller/views_widgets/summary.py`       | `argus/backend/tests/widgets/test_summary_widget.py`              | `complete` | 6 tests covering `/api/v1/views/widgets/summary/{versioned_runs,runs_results}`. Empty + unknown-id + unknown-test paths plus full data-shape: seeded SCT run with packages surfaces correct version key in `versions`, and `runs_results` returns the seeded `P99 read` cell value/status from `generic_result_data_v1`.                                                                                                                                                                                                                                                                                                                                                    |
| 16  | `argus/backend/controller/views_widgets/pytest.py`        | `argus/backend/tests/widgets/test_pytest_widget.py`               | `complete` | 7 tests covering `/api/v1/views/widgets/pytest/{view, view/<id>/results, release/<id>/results, <path:test_name>/<id>/fields}`. Empty-data path verified; full data-shape: a seeded `pytest_v2` row with `user_fields` surfaces in `result_filter` hits + pieChart counts (with required `status[]` query arg to satisfy clustering-key restriction), and `get_user_fields_for_result` returns the persisted user_fields.                                                                                                                                                                                                                                                    |
| 17  | `argus/backend/controller/views_widgets/nemesis_stats.py` | `argus/backend/tests/widgets/test_nemesis_stats_widget.py`        | `complete` | 4 tests covering `/api/v1/views/widgets/nemesis_data`. Empty/unknown/missing paths plus full data-shape: seeded SCT run with finalized `SCTNemesis` (`succeeded`) appears in `nemesis_data` with version derived from the run's `scylla-server` package.                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 18  | `argus/backend/controller/views_widgets/graphs.py`        | `argus/backend/tests/widgets/test_graphs_widget.py`               | `complete` | 4 tests covering `/api/v1/views/widgets/graphs/graph_views`. Empty/unknown/no-graph-views paths plus full data-shape: seeded `ArgusGraphView` for a test surfaces in `tests_details` with the test name (graph payload itself depends on `create_chartjs` chains; presence in `tests_details` confirms the controller branch).                                                                                                                                                                                                                                                                                                                                              |
| 19  | `argus/backend/controller/views_widgets/graphed_stats.py` | `argus/backend/tests/widgets/test_graphed_stats_widget.py`        | `complete` | 9 tests covering `/api/v1/views/widgets/{graphed_stats, runs_details}`. Empty/unknown/error paths plus full data-shape: seeded SCT run appears in `test_runs` with version + run_id; finalized nemesis appears in `nemesis_data`; `runs_details` returns version (`<version>-<date>`) and surfaces a linked `GithubIssue` via `IssueLink`.                                                                                                                                                                                                                                                                                                                                  |
| 20  | `argus/backend/controller/auth.py`                        | `argus/backend/tests/auth/test_auth.py`                           | `complete` | 13 tests covering `/register`, `/login` GET (authenticated + redirect_target) and POST (password success + password method disabled), `/login/cf` (no JWT path + valid JWT happy path via `mock_cf_access_payload` for an existing @scylladb.com user), `/logout`, `/profile/api/token/generate`, `/admin/impersonate` GET + POST (missing user_id + full impersonate→stop round-trip asserting `original_user`/`user_id` session transitions), `/admin/impersonate/stop` (no active session error).                                                                                                                                                                        |
| 21  | `argus/backend/controller/main.py` (profile only)         | `argus/backend/tests/main_api/test_profile.py`                    | `partial`  | 20 tests covering `/profile/update/{name,username,email,password,picture}` (success + validation paths), `/storage/picture/<id>` (success, 404 on missing file, 500 on unknown id), `/profile/oauth/github` (bad CSRF state, success with `mock_github_callback` populating `first_run_info` and using `redirect_target`, callback exception path), `/profile/create` (registration_allowed gate, happy-path POST creating user + session login, `lock_user_email` mismatch error). Deferred: `/profile/`, `/profile/jobs`, `/profile/schedules` template renders (intentionally skipped — pure template smoke).                                                            |
| 22  | `argus/backend/controller/views_widgets/highlights.py`    | `argus/backend/tests/view_widgets/test_highlights_api.py`         | `complete` | All 12 controller endpoints covered by pre-existing 17-test module (verified). No new work required.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
