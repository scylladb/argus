...
- **Server-generated keypairs** — Argus generates ed25519 keypairs server-side. Only the public key is stored in DB; the private key is returned to the client once over HTTPS and discarded. This follows the same model as AWS EC2 keypairs.
...
1. **Transport security** — private key transmitted over HTTPS (TLS-encrypted via Cloudflare).
...
4. **No private key stored server-side** — only public key + fingerprint in DB. DB compromise does not leak private keys.
5. **Real-time key validity** — `AuthorizedKeysCommand` queries Argus API on each SSH connection, so key validity is always current. No file writes, no sync races.
6. **Host key verification** — API returns proxy host key fingerprint; client uses `StrictHostKeyChecking=yes` with a generated known_hosts file (no TOFU).
7. **Instant revocation** — deleting a key row from DB takes immediate effect on next SSH connection attempt.
...
Client                          Argus Backend (via Cloudflare)        Proxy Host
  ...
  |   (auth_token, ttl_minutes?)       |                                |
  |                                    |-- generate keypair             |
  |                                    |-- store PUBLIC key in DB       |
  |                                    |-- discard private key          |
  |<-- {private_key, proxy_host,     |                                |
  ...
  |   (using received private_key, verifying host_key_fingerprint)      |
  |                                    |                                |
...
```
Client
  |-- Check ~/.ssh/id_argus_proxy (key comment contains expiry timestamp)
  |-- ssh -N -L ... (using cached key, verifying host fingerprint)
  |-- HTTP requests via localhost:local_port
  |
  |-- If tunnel fails: re-call /client/ssh/tunnel, get fresh key+config
...
2. On first HTTP request, calls `_ensure_tunnel()`:
   - Checks `~/.ssh/id_argus_proxy` for cached key (expiry in key comment)
   - If no cache or expired: calls `POST /api/v1/client/ssh/tunnel` to get key+config, saves key to `~/.ssh/id_argus_proxy` (mode 0600)
...
### Key Lifecycle
- **One key per session** — each `POST /client/ssh/tunnel` generates a fresh keypair. Private key returned once, only public key stored in DB.
...
- Before using cached key: parse expiry from comment → if expired, re-request
- **Mid-session revocation** — SSH authenticates only at connection establishment. A deleted key does NOT kill an active tunnel. The revocation takes effect on next reconnect, at which point the client calls the API and gets a fresh key.
...
class SSHTunnelKey(Model):
    ...
    public_key = columns.Text()        # stored server-side for AuthorizedKeysCommand
...
Note: private key is NOT stored. Generated, returned to client once, then discarded server-side.

**Rows inserted with ScyllaDB TTL** — default 24 hours (86400s), configurable via client-provided `ttl_seconds`. ScyllaDB automatically deletes expired rows, no manual cleanup needed. The `expires_at` field is informational (returned to client so it knows when to re-request).
...
- `request_tunnel(user: User, ttl_seconds: int = None) -> dict`:
  1. Generate ed25519 keypair via Python `cryptography` library
  2. Store `SSHTunnelKey` in DB (public key only) with ScyllaDB TTL (default 86400s / 24h, or client-provided `ttl_seconds`). Set `expires_at = now_utc + ttl` for informational purposes.
  3. Get active `ProxyTunnelConfig`
  4. Return `{private_key, proxy_host, proxy_port, proxy_user, target_host, target_port, host_key_fingerprint, expires_at}` (all datetimes UTC ISO-8601)
  5. Private key is NOT stored — returned to client and discarded
...
def request_tunnel():
    ...
    result = TunnelService().request_tunnel(g.user, ttl_seconds=ttl_seconds)
...
```
1. Check `~/.ssh/id_argus_proxy` for cached key (parse expiry from key comment)
2. If cached and not expired and not `force_refresh`: return config from key + `~/.ssh/config`
3. Otherwise: `POST {base_url}/api/v1/client/ssh/tunnel` with auth token
4. Save private key to `~/.ssh/id_argus_proxy` (mode 0600) with expiry as comment
5. On any failure: log warning, return None
...
- Test keypair generation (only public key stored, private key not in DB)
...
| `argus/backend/service/tunnel_service.py` | **CREATE** | Key generation, authorized keys query, tunnel API logic |
