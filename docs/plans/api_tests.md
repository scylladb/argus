...
| 15  | `argus/backend/controller/views_widgets/summary.py`       | `argus/backend/tests/widgets/test_summary_widget.py`              | `partial`  | 4 tests covering `/api/v1/views/widgets/summary/{versioned_runs,runs_results}`. Empty-view path returns `{versions: {}, test_info: {}}`; unknown view_id surfaces `DoesNotExist`. `runs_results` exercised with empty payload (returns `{}`) and unknown test_id/run_id (returns `[]` for that method). Full data-shape coverage deferred (requires SCT runs with packages + generic_result_metadata seeding).                                                                                                             |
| 16  | `argus/backend/controller/views_widgets/pytest.py`        | `argus/backend/tests/widgets/test_pytest_widget.py`               | `partial`  | 5 tests covering `/api/v1/views/widgets/pytest/{view, view/<id>/results, release/<id>/results, <path:test_name>/<id>/fields}`. Empty-data path verified (returns `total=0`, empty `hits`/`pieChart`); tests pass `before`/`after` query params to avoid empty-result IndexError in `prepare_bar_chart`. User-fields endpoint exercised for unknown-key (returns `{}`) and invalid isoformat id (error). Full data-shape coverage deferred (requires `pytest_v2` + `pytest_user_field` seeding).                            |
| 17  | `argus/backend/controller/views_widgets/nemesis_stats.py` | `argus/backend/tests/widgets/test_nemesis_stats_widget.py`        | `partial`  | 3 tests covering `/api/v1/views/widgets/nemesis_data`. Empty-view path returns `{nemesis_data: []}`; unknown view_id surfaces `DoesNotExist`; missing `view_id` query arg errors. Full data-shape coverage deferred (requires SCT runs + nemesis rows seeding).                                                                                                                                                                                                                                                            |
| 18  | `argus/backend/controller/views_widgets/graphs.py`        | `argus/backend/tests/widgets/test_graphs_widget.py`               | `partial`  | 3 tests covering `/api/v1/views/widgets/graphs/graph_views`. Empty-view path returns `{}`; view containing a test with no graph_views returns empty list per test_id and empty `tests_details`; unknown view_id surfaces `DoesNotExist`. Full data-shape coverage deferred (requires `ArgusGraphView` + result tables seeding).                                                                                                                                                                                            |
| 19  | `argus/backend/controller/views_widgets/graphed_stats.py` | `argus/backend/tests/widgets/test_graphed_stats_widget.py`        | `partial`  | 6 tests covering `/api/v1/views/widgets/{graphed_stats, runs_details}`. Empty-view path returns `{test_runs: [], nemesis_data: []}`; unknown view_id surfaces `DoesNotExist`. `runs_details` exercised for empty list (`{}`), unknown run_ids (returns placeholder dict per id with `version="unknown"`, `issues=[]`), missing `run_ids` and non-list `run_ids` error paths. Full data-shape coverage deferred (requires SCT runs + IssueLink + GithubIssue seeding).                                                      |
