...
| 14  | `argus/backend/controller/planner_api.py`                 | `argus/backend/tests/planner_api/test_planner_api.py`             | `partial`  | 16 tests covering `/api/v1/planning/{,/plan/{create,<id>/get,update,<id>/owner/set,<id>/resolve_entities,<id>/delete,<id>/copy/check}, /release/<id>/{all,gridview}, /search, /group/<id>/explode}`. Verified via paired GETs (including `/api/v1/views/get` for plan-view linkage on delete). Auto-created view per plan covered (delete+keep variants). Copy eligibility tested with empty target release (`failed` status, missing tests reported). `/plan/copy` and `/plan/trigger` deferred (Jenkins/copy mocking).                                                                                                                                                    |
