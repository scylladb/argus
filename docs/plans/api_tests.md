...
| 20  | `argus/backend/controller/auth.py`                        | `argus/backend/tests/auth/test_auth.py`                           | `partial`  | Covered: `/register`, `/login` GET (authenticated + redirect_target), `/logout`, `/profile/api/token/generate`, `/login/cf` (no JWT path), `/admin/impersonate` GET + POST missing user_id, `/admin/impersonate/stop` (no active session). Deferred: password login flow, CF JWT happy path, full impersonation flow.                                                                                                                                                                                                                                                                                                                                                       |
