...
class EventSimilarityProcessorV2:
    ...
    def _clear_stale_cache(self):
        def cmp_ts(ts: datetime | float):
            match ts:
                case float():
                    return ts < (time.time() + self.cache_clear_timer)
                case datetime():
                    return ts.timestamp() < (time.time() + self.cache_clear_timer)

        for [run_id, severity], events in self.similar_event_cache.items():
            full_cache = [event for event in events if cmp_ts(getattr(event, "added_ts", 0.0))]
            if len(full_cache) == 0:
                self.similar_event_cache.pop((run_id, severity))
