...
class ArgusReporter(object):  # pylint: disable=too-many-instance-attributes
    def __init__(self, config):
        ...
        if config.getoption("post_reports") is not None:
            self.post_reports = config.getoption("post_reports")
        else:
            self.post_reports = False

        self.base_url = config.getoption("base_url")
        self.api_key = config.getoption("api_key")
        self.extra_headers = config.getoption("extra_headers") or dict()
        ...
        if not self.run_id:
            self.run_id = os.environ.get("ARGUS_RUN_ID", None)
        self.test_type = config.getoption("test_type")
        if not self.test_type:
            self.test_type = os.environ.get("ARGUS_TEST_TYPE", None)
    ...
    def post_to_argus(self, test_data):
        if self.post_reports and not self.is_slave:
            try:
                ...
                res = self.argus_client.post(endpoint="/testrun/pytest/result/submit", location_params={}, body=request_data)
    ...
    def fetch_test_duration(
        ...
        def get_test_stats(test_id):            
            try:
                res = self.argus_client.get(endpoint=f"/testrun/pytest/{test_id}/stats/duration/avg", location_params=dict(), params=self.slices_query_fields)
                res.raise_for_status()
                test_data = next(iter(res.json()["response"].values()), dict())
                return dict(
                    test_name=test_id,
                    duration=test_data["duration"]["avg"]
                )
    ...
    def pytest_collection_finish(self, session):
        ...
        if self.config.getoption("slices"):
            assert (
                self.default_test_time and self.max_splice_time
            ...
            slices = self.make_test_slices(
                test_history_data, max_slice_duration=self.max_splice_time * 60
            )
