<script lang="ts">
    ...
    function changeVersion(direction) {
        currentVersionIndex += direction;
        if (currentVersionIndex < 0) currentVersionIndex = versions.length - 1;
        if (currentVersionIndex >= versions.length) currentVersionIndex = 0;
        fetchRunResults();
    }
    ...
    {:else}
        <div class="card-header bg-primary text-white">
            <div class="d-flex justify-content-between align-items-center">
                <h3 class="mb-0">Test Results Summary</h3>
                <div class="d-flex align-items-center">
                    <button class="btn btn-outline-light me-2" on:click={() => changeVersion(-1)}>
                        <Fa icon={faChevronLeft}/>
                    </button>
                    <select class="form-select" bind:value={currentVersionIndex} on:change={fetchRunResults}>
                        {#each versions as version, index}
                            <option value={index}>{formatVersion(version)}</option>
                        {/each}
                    </select>
                    <button class="btn btn-outline-light ms-2" on:click={() => changeVersion(1)}>
