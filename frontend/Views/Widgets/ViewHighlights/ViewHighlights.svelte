<script lang="ts">
    ...
    import {faPlus, faChevronDown, faChevronRight, faChevronUp} from "@fortawesome/free-solid-svg-icons";
    ...
    function shouldCollapseGroup(name: string): boolean {
        const activeItems = getGroupItems(name, false);
        return activeItems.length > 0 && activeItems.every(item => item.isCompleted);
    }
    ...
    function getOrCreateGroupState(name: string): GroupState {
        ...
        return {
            ...
            collapsed: shouldCollapseGroup(name),
            touched: false,
        };
    }
    ...
    function syncGroupStates() {
        ...
        for (const name of sorted) {
            ...
            const shouldCollapse = shouldCollapseGroup(name);
    ...
    function isGroupCollapsed(name: string): boolean {
        return groupStates[name]?.collapsed ?? shouldCollapseGroup(name);
    }
                ...
                <ul class="list-group">
                    {#each groupNames as groupName (groupName)}
                        {#if shouldShowActiveGroup(groupName)}
                            <li class="list-group-item bg-body-secondary fw-semibold d-flex align-items-center">
