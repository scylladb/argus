...
<script lang="ts">
    ...
    const parseFilter = function (filter: string): [string, boolean] {
        return [filter.replace(/!/, ""), filter[0] == "!"];
    };
    ...
    const handleFilterClick = function (event: CustomEvent) {
        ...
        if (widgetState.filters.includes(filter)) return;
        widgetState.filters.push(filter);
                ...
                <div class="bg-white rounded p-2">
                    ...
                    <div class="bg-light-one p-1 d-flex flex-wrap rounded">
                        {#each widgetState.filters as filter}
                            {@const [filterName, negated] = parseFilter(filter)}
                            <div class="btn-group me-1 mb-1">
