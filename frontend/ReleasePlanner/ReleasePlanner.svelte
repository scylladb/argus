...
{#if createFromPlan}
    <ModalWindow on:modalClose={() => {selectedPlan = undefined; createFromPlan = false;}}>
        <div slot="title">
            Creating new plan from <span class="fw-bold">{selectedPlan.name}</span>
        </div>
        <div slot="body">
            <ReleasePlanCreator mode="createFrom" {release} plan={selectedPlan} on:planCreated={() => {
...
{#if editingPlan}
    <ModalWindow on:modalClose={() => {selectedPlan = undefined; editingPlan = false;}}>
        ...
        <div slot="body">
            <ReleasePlanCreator mode="edit" {release} plan={selectedPlan} on:planUpdated={() => {
                editingPlan = false;
                selectedPlan = undefined;
                fetchAllPlans();
            }}/>
