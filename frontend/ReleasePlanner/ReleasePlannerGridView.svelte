<script>
     ...
     * }}
     */
    const shouldFilter = function (entity, filterEnabled) {
        const planned = existingPlans
            .map(({tests, groups}) => [...tests, ...groups])
            .reduce((acc, val) => [...acc, ...val], []);

        return filterEnabled ? planned.includes(entity.id) : false;
    };
        ...
        <div>
            <div class="form-check form-switch">
                <input
                    ...
                    bind:checked={filterExisting}
                    id="gridViewFilterExisting"
                />
                <label
                    class="form-check-label"
                    for="gridViewFilterExisting">Hide already planned</label
                >
            </div>
        </div>
        <div>
                            ...
                            {@const testStats = releaseStats?.["groups"]?.[test.group_id]?.["tests"]?.[test.id] ?? {}}
                                    ...
                                    >
                                        <div
                                            ...
                                            class:d-none={shouldFilter(test, filterExisting)}
                                        >
