...
export const resourceCost = function (
    ...
    if (resource.state === RUNNING_STATE && isKnownAmount(info.price_per_hour) && isKnownAmount(info.creation_time)) {
        ...
        // A creation_time in the future means the clocks disagree, not that the resource
        // has run for free — reporting 0 here would be exactly the false zero we avoid.
...
export const isLiveCost = function (
    resource: CostResource,
    nowSeconds: number = Date.now() / 1000
): boolean {
    const info = resource?.instance_info;
    if (!info) return false;
    if (resource.state !== RUNNING_STATE || isKnownAmount(info.cost)) return false;
    // Only "live" when there is actually a live figure to label.
    return resourceCost(resource, nowSeconds) !== null;
};
