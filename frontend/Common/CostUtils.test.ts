...
describe("resourceCost", () => {
    ...
    it("treats a creation time in the future as unknown, not free", () => {
        // Clock skew between the Argus server and the browser must not become a $0.00.
        expect(resourceCost(running(1.5, -1), NOW)).toBeNull();
    });
...
describe("isLiveCost", () => {
    ...
    it("is false when there is no live figure to label yet", () => {
        expect(isLiveCost(running(1.5, -1), NOW)).toBe(false);
    });
});
...
describe("runCostSummary", () => {
    ...
    it("counts a future-dated running resource as unknown, not as a known zero", () => {
        // A single skewed resource must leave the run with nothing known, so the details
        // page renders "No cost data" rather than "$0.00".
        const summary = runCostSummary([running(1.5, -1)], NOW);
        expect(summary.known).toBe(0);
        expect(summary.unknown).toBe(1);
        expect(summary.partial).toBe(true);
    });
