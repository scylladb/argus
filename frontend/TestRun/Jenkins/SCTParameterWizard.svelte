            ...
            params: {
                ...
                scyllaBranchBYO: {
                    ...
                    validate: async function (params) {
                        ...
                        // Only reachable once the repository itself is known good -- otherwise the
                        // repository field is already reporting the problem and we would duplicate
                        // its error here (and waste a GitHub call).
                        const repo = params.byo_scylla_repo;
                        const repoDefinition = paramDefinitions.scyllaVersion.params.scyllaRepoBYO;
                        if (!GIT_REPO_RE.test(repo ?? "") || !repoDefinition.validated) {
                            return true;
                        }
