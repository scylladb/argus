...
#   --target-host  <str>   Private IP of the Argus backend
#
# Optional flags:
...
ARGUS_API_URL="http://${TARGET_HOST}:${TARGET_PORT}"
...
# Token is embedded directly — sourcing an env file does not work when sshd
# invokes this as AuthorizedKeysCommandUser nobody.
...
if sudo -u nobody /usr/local/bin/argus-authorized-keys > /dev/null 2>&1; then
    ...
    echo "WARNING: argus-authorized-keys returned non-zero — check --token and --target-host." >&2
fi
