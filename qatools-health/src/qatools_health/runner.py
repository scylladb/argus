...
type Driver = tuple[CheckState, HealthCheckStatus] | None
...
class HealthCheckRunner:
    ...
    called start. A stopped runner holds no check, and start runs it again.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._announced: HealthCheckStatus | None = None
    ...
    def register(self, check: HealthCheck, *, on_change: OnChange | None = None) -> HealthCheckSubscription:
        ...
        with self._lock:
            ...
            state, subscription = self._subscribe(check, on_change)
    ...
    async def stop(self) -> None:
        ...
        The runner keeps the last result of every check for the snapshot and
        the scrape, and sends no aggregate callback while it stops. The next
        register or start drops those checks, so a later start probes again
        from a clean state.
        """
        self._started = False
        with self._lock:
    ...
    def _drop_retired(self) -> None:
        self._states = {identity: state for identity, state in self._states.items() if not state.retired}

    def _subscribe(
        self,
        check: HealthCheck,
        on_change: OnChange | None = None,
    ) -> tuple[CheckState, HealthCheckSubscription]:
    ...
    def _retire(self, state: CheckState) -> None:
        if state.retired:
            return
        with self._lock:
            if state.subscriptions or state.retired:
                return
            state.retired = True
            if self._states.get(state.identity) is state:
                self._states.pop(state.identity)
    ...
    async def _pause(self, state: CheckState) -> None:
        loop = asyncio.get_running_loop()
        finished = loop.time()
        while True:
            remaining = finished + state.check.interval - loop.time()
            if remaining <= 0:
                return
            state.wake.clear()
            try:
    ...
    async def _publish(self, state: CheckState, result: HealthCheckResult, duration: float) -> None:
        with self._lock:
            previous = state.status if state.result is not None else None
            now = self._clock()
    ...
    def _log_transition(
        self,
        state: CheckState,
        previous: HealthCheckStatus | None,
        result: HealthCheckResult,
    ) -> None:
        detail = result.message or result.error or ""
        reference = HealthCheckStatus.HEALTHY if previous is None else previous
        level = logging.WARNING if is_worse(result.status, reference) else logging.INFO
        before = "PENDING" if previous is None else previous
        LOGGER.log(level, "health check %s %s -> %s: %s", state.check.name, before, result.status, detail)
    ...
    async def _notify_aggregate(self) -> None:
        ...
        now = self._clock()
        with self._lock:
            if any(state.result is None for state in self._states.values()):
                return
            aggregate, driver = self._compute(now)
            if aggregate is self._announced:
                return
            self._announced = aggregate
            reason = self._reason(driver)
            statuses = {state.check.name: state.effective_status(now) for state in self._states.values()}
        if self._on_change is not None:
            await call_back(self._on_change, aggregate, reason, statuses, subject=self.service)
