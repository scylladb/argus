...
from qatools_health.status import AGGREGATE_GAUGE_VALUE, DEPENDENCY_GAUGE_VALUE, HealthCheckStatus
...
class HealthMetricsCollector:
    ...
    def describe(self) -> Iterable[Metric]:
        """Return the metric families without reading the runner.

        The registry reads the names to refuse a second collector that emits
        the same families, which would make the exposition invalid.
        """
        return build_families(
            RunnerSnapshot(service="", version="", aggregate=HealthCheckStatus.HEALTHY, runner_up=False)
        )

    def collect(self) -> Iterable[Metric]:
        """Read one snapshot and build the metric families from it."""
        return build_families(self._read_snapshot())


def build_families(snapshot: RunnerSnapshot) -> tuple[Metric, ...]:
    """Build every metric family from one snapshot."""
