...
type OnChange = Callable[[HealthCheck, HealthCheckResult], Awaitable[None] | None]
type OnGroupChange = Callable[[HealthCheckStatus, str, Mapping[str, HealthCheckStatus]], Awaitable[None] | None]
...
class HealthCheckGroup:
    ...
    on a change of that worst status, not on every member change, and never
    before every member has published a first result.
    """
    ...
    @property
    def statuses(self) -> dict[str, HealthCheckStatus]:
        """The status of every member right now, by check name."""
        return {member.check.name: member.status for member in self._members}

    @property
    def settled(self) -> bool:
        """Whether every member has published a first result."""
        return all(member.result is not None for member in self._members)
    ...
    async def wait_for(
        ...
        *statuses: HealthCheckStatus,
        ...
        wanted = frozenset(statuses)
        status = self.status
        if status in wanted and self.settled:
            return status
    ...
    async def _member_changed(self, _: HealthCheckSubscription) -> None:
        if self._closed or not self.settled:
            return
        status = self.status
        self._resolve(status)
        if self._seen is status:
            return
        self._seen = status
        if self._on_change is not None:
            await call_back(self._on_change, status, self._reason(), self.statuses, subject="the dependency group")
