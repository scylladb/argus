...
def is_identity_field(key: str, value: Any, owner: object) -> bool:
    ...
    policy keyword tunes the check. A callable declared on the class is
    behavior, and so is a method bound to the check itself. A callable handed
    to one instance, such as the function a decorated check awaits or the
    getter a staleness check reads, is configuration. Configuration is what
    names a dependency.
    """
    ...
    if isinstance(owner, type):
        return False
    return getattr(value, "__self__", None) is not owner
...
class HealthCheck(ABC):
    ...
    def identity(self) -> tuple[object, ...]:
        ...
        which is how a check keeps a transport object out of its identity. A
        function stored on the instance stays in, so two decorated functions
        are two dependencies and one function decorated twice is one.
        """
    ...
    def identity_fields(self) -> dict[str, Any]:
        ...
        for source in (*reversed(type(self).__mro__), self):
            for key, value in vars(source).items():
                if is_identity_field(key, value, source):
                    fields[key] = value
        return fields
