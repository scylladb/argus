...
It imports nothing from Argus. It depends on `prometheus-client`, `httpx` and
`aiosqlite`, and on nothing else. It needs Python 3.13 or later.

Every public module, class and method has a docstring. This file is the guide
and the API reference. The `examples/` directory holds runnable services that
use the package.
...
Both examples run a local HTTP server as the dependency, take it down for a
moment, and bring it back. They need no network and no credentials.

| Example | Shows |
| --- | --- |
| `examples/service.py` | A whole service: a dedicated `HttpHealthCheck` subclass, TCP, SQLite, staleness and inline checks, an `OPTIONAL` check, per-check and runner-level callbacks, and the Prometheus exposition. Pass `--port 9464` to serve `/metrics`. |
| `examples/gated_worker.py` | A publisher that sends work only while its group of dependencies is HEALTHY, and an auditor that subscribes to the same upstream with its own callback. Both hear each transition from one probe loop. |

```
.venv/bin/python examples/service.py --duration 10
.venv/bin/python examples/gated_worker.py --duration 6
```

`tests/test_examples.py` runs both, so an example that stops working fails CI.
...
one probe URL and one credential. `JenkinsApiHealthCheck(client=jenkins_client)`
carries no credential of its own, so it is a different dependency from both, and
registering it beside them under the default name raises.
...
check keeps a transport object out of its identity. A method declared on the
class stays out. A callable stored on the instance stays in, so the function
behind an inline check and the getter of a `StalenessHealthCheck` each name a
dependency. `HttpHealthCheck` holds the
...
identity holds the decorated function, so registering the same decorated
function twice yields one check, and two decorated functions are two checks.
...
| `statuses` | The status of every member, by check name. |
| `settled` | Whether every member has published a first result. |
...
group with one member reads exactly like that member. The group waits for every
member to publish a first result, so neither `wait_for()` nor the callback
resolves on a member that has not run yet.
...
| `stop()` | Cancel every armed timer, await the checks still in flight, call `aclose()` on each check, and end every open subscription. It keeps the last results and sends no aggregate callback while it stops. |
...
A stopped runner keeps the last result of every check, so a scrape after
`stop()` shows the final state with `healthcheck_runner_up` at 0. The next
`register()` or `start()` drops those checks, and that `start()` probes from a
clean state.

One registry holds one runner. `register_collector()` raises `ValueError` for a
second runner in the same registry, because two collectors with the same metric
families make the exposition invalid.
...
OnGroupChange = Callable[[HealthCheckStatus, str, Mapping[str, HealthCheckStatus]], Awaitable[None] | None]
```
...
The runner-level `on_change` fires on every change of the aggregate status. It
gets the new status, a reason that names the check behind it, and the status of
every registered check by name. It stays silent until every registered check has
published a first result, so the first-run spread never produces an event whose
reason names a check that has not run. It is the one way a service acts on a
transition the package knows nothing about, such as writing the systemd status
line.

```python
def on_service_change(status, reason, statuses):
    failing = sorted(name for name, value in statuses.items() if value is not HealthCheckStatus.HEALTHY)
    notify(f"STATUS={status}: {reason}, not healthy: {', '.join(failing) or 'none'}")


runner = HealthCheckRunner(service="zeus", on_change=on_service_change)
```

The group callback takes the same three arguments, over the members of that
group.

Two subscriptions to one dependency each keep their own callback. Two components
that register `GitHubApiHealthCheck(GH_TOKEN)` share one probe loop, and both
...
- A status change is logged once: INFO on recovery, WARNING on failure, with the
  check name, the old status, the new status and the message. The first result of
  a check is logged from `PENDING`, so a GitHub token that is rate limited from
  startup writes `health check github_api PENDING -> UNHEALTHY: the GitHub rate
  limit is spent, ...`. A dependency down for a day writes one line, not one per
  interval. This log is the only place the message reaches an operator.
...
shown. A client keeps its own `auth` unless the check gets credentials of its
own. The identity holds the probe URL and the credentials, so the two forms reach
one check only when both carry the same credentials. A class that gets neither a
base URL nor a client carrying one raises.
...
request on itself. A body that holds no core budget, such as an HTML maintenance
page, is UNHEALTHY with that message.

The check follows the GitHub rate limit rules. It reports UNHEALTHY for the
whole wait, and it sends no request until the wait ends:

| GitHub answer | Wait |
| --- | --- |
| 403 or 429 with `x-ratelimit-remaining: 0` | Until `x-ratelimit-reset`, or 60 seconds when the header is absent |
| 403 or 429 with `retry-after` | The `retry-after` seconds |
| 429 with neither header, or 403 whose message names a rate limit | 60 seconds |
| `/rate_limit` body with a core budget of 0 | Until the core `reset` |
| Any other 403 | No wait, the next interval probes again |

The same rules apply to the `/user` read. A probe during the wait returns at once
with a message such as `rate_limit answered 403, the GitHub primary rate limit
is spent, resets in 5m, no request sent`. The first probe after the wait sends a
request again.

`AnthropicApiHealthCheck` runs two probes. The first reads the `Claude API
(api.anthropic.com)` component from `https://status.claude.com/api/v2/summary.json`.
...
verdict, and the check falls through to the key probe. A status page that
answers with a body that is not the expected JSON is treated the same way.
...
| Class | Reports |
| --- | --- |
| `HttpHealthCheck(url, method="GET", expect=range(200, 300), latency_budget=None, headers=None, auth=None, client=None)` | Reachability of an HTTP dependency. Over the latency budget gives DEGRADED. A wrong status, a redirect or a transport error gives UNHEALTHY. |
| `TcpHealthCheck(host, port)` | A port accepts a connection. For a tunnel, or a database with no cheap query. |
| `BinaryHealthCheck(binary, version_args=("--version",))` | An executable is on `PATH` and answers. A run cut off by the timeout kills the process. |
...
`HttpHealthCheck` grades the first answer and does not follow a redirect. A
redirect to a login page is a failed probe, not a healthy one. Pass the final
URL, or pass `expect` with the redirect codes the dependency uses on purpose.
