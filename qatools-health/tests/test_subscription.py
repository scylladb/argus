...
async def test_the_group_callback_names_the_member_behind_the_status(clock):
    ...
    runner.register_all([good, bad], on_change=lambda status, reason, statuses: seen.append((status, reason, statuses)))
    runner.start()
    await settle(good)
    await settle(bad)
    await spin()
    assert seen == [(UNHEALTHY, "b unhealthy", {"a": HEALTHY, "b": UNHEALTHY})]
    await runner.stop()
...
async def test_the_group_reports_a_healthy_reason(clock):
    ...
    runner.register_all([check], on_change=lambda status, reason, statuses: seen.append(reason))
    runner.start()
    await settle(check)
    await spin()
...
async def test_a_repeated_status_delivers_no_callback(clock):
    seen = []
    runner = build(clock)
    check = ScriptedCheck([True, HealthCheckResult.healthy("still fine"), True, True], name="jira", interval=0.001)
    runner.register(check, on_change=lambda subject, result: seen.append(result.status))
    runner.start()
...
async def test_a_repeated_group_status_delivers_no_callback(clock):
    seen = []
    runner = build(clock)
    first = ScriptedCheck([True], name="a", interval=0.001)
    second = ScriptedCheck([True], name="b", interval=0.001)
    runner.register_all([first, second], on_change=lambda status, reason, statuses: seen.append(status))
    runner.start()
    await settle(second, calls=5)
    await spin()
    await runner.stop()
    assert seen == [HEALTHY]


async def test_two_subscribers_of_one_dependency_both_hear_the_failure(clock):
    first_seen, second_seen = [], []
    runner = build(clock)
    check = ScriptedCheck([True, HealthCheckResult.unhealthy("token revoked")], name="github_api", interval=0.001)
    first = runner.register(check, on_change=lambda subject, result: first_seen.append(result.status))
    second = runner.register(
        ScriptedCheck(name="github_api"), on_change=lambda subject, result: second_seen.append(result.status)
    )
    runner.start()
    await first.wait_for(UNHEALTHY, timeout=2)
    await second.wait_for(UNHEALTHY, timeout=2)
    await spin()
    await runner.stop()
    assert first.check is second.check
    assert check.calls >= 2
    assert first_seen[:2] == [HEALTHY, UNHEALTHY]
    assert second_seen[:2] == [HEALTHY, UNHEALTHY]


async def test_two_services_sharing_one_github_check_both_hear_the_failure(clock):
    budget = {"remaining": 4200}

    def handle(request):
        return httpx.Response(200, json={"resources": {"core": {"remaining": budget["remaining"], "limit": 5000}}})

    client = httpx.AsyncClient(transport=httpx.MockTransport(handle))
    zeus_seen, maia_seen = [], []
    runner = build(clock)
    zeus = runner.register(
        GitHubApiHealthCheck("token", client=client, interval=0.01),
        on_change=lambda check, result: zeus_seen.append(result.status),
    )
    maia = runner.register(
        GitHubApiHealthCheck("token", client=client, interval=0.01),
        on_change=lambda check, result: maia_seen.append(result.status),
    )
    runner.start()
    await zeus.wait_for(HEALTHY, timeout=2)
    budget["remaining"] = 0
    await zeus.wait_for(UNHEALTHY, timeout=2)
    await maia.wait_for(UNHEALTHY, timeout=2)
    await spin()
    await runner.stop()
    assert runner.snapshot().runner_up is False
    assert zeus_seen == [HEALTHY, UNHEALTHY]
    assert maia_seen == [HEALTHY, UNHEALTHY]


async def test_the_group_stays_silent_until_every_member_has_run(clock):
    seen = []
    runner = build(clock)
    slow = ScriptedCheck([True], name="slow", delay=0.1)
    fast = ScriptedCheck([HealthCheckResult.unhealthy("gone")], name="fast")
    group = runner.register_all([fast, slow], on_change=lambda status, reason, statuses: seen.append(status))
    runner.start()
    await settle(fast)
    await spin()
    assert fast.calls == 1
    assert seen == []
    assert await group.wait_for(UNHEALTHY, timeout=2) is UNHEALTHY
    await spin()
    await runner.stop()
    assert seen == [UNHEALTHY]


async def test_a_group_wait_does_not_resolve_on_a_member_that_never_ran(clock):
    runner = build(clock)
    slow = ScriptedCheck([True], name="slow", delay=0.2)
    fast = ScriptedCheck([True], name="fast")
    group = runner.register_all([fast, slow])
    runner.start()
    waiter = asyncio.create_task(group.wait_for(UNHEALTHY, HEALTHY))
    await settle(fast)
    await spin()
    assert waiter.done() is False
    assert await asyncio.wait_for(waiter, 2) is HEALTHY
    await runner.stop()


async def test_the_group_callback_reads_every_member_status(clock):
    seen = []
    runner = build(clock)
    checks = [
        ScriptedCheck([True], name="jenkins_api"),
        ScriptedCheck([HealthCheckResult.degraded("slow")], name="jira_api"),
        ScriptedCheck([True], name="gh"),
    ]
    group = runner.register_all(checks, on_change=lambda status, reason, statuses: seen.append(dict(statuses)))
    runner.start()
    await group.wait_for(DEGRADED, timeout=3)
    await spin()
    await runner.stop()
    assert seen == [{"jenkins_api": HEALTHY, "jira_api": DEGRADED, "gh": HEALTHY}]
