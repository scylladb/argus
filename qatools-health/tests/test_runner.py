...
from qatools_health import HealthCheckResult, HealthCheckRunner, HealthCheckStatus, Severity, healthcheck
...
async def test_on_change_fires_once_per_transition_and_names_the_check(clock):
    ...
    runner = build(clock, check, on_change=lambda status, reason, statuses: seen.append((status, reason, statuses)))
    runner.start()
    await settle(check, calls=2)
    await asyncio.sleep(0.02)
    await runner.stop()
    assert seen == [
        (HEALTHY, "every dependency healthy", {"jira": HEALTHY}),
        (DEGRADED, "jira unhealthy", {"jira": UNHEALTHY}),
    ]
...
async def test_on_change_awaits_a_coroutine_callback(clock):
    ...
    async def record(status, reason, statuses):
        seen.append(status)
...
async def test_a_failing_on_change_does_not_reach_the_service(clock, caplog):
    def explode(status, reason, statuses):
...
async def test_the_runner_stays_silent_until_every_check_has_run(clock):
    seen = []
    slow = ScriptedCheck([True], name="slow", delay=0.1)
    fast = ScriptedCheck([HealthCheckResult.unhealthy("gone")], name="fast")
    runner = build(clock, fast, slow, on_change=lambda status, reason, statuses: seen.append((status, reason)))
    runner.start()
    await settle(fast)
    await spin()
    assert seen == []
    ...
    await runner.stop()
    assert seen == [(DEGRADED, "fast unhealthy")]


async def test_the_runner_callback_reads_every_check_status(clock):
    seen = []
    scylla = ScriptedCheck([True], name="scylla", severity=Severity.CRITICAL)
    jira = ScriptedCheck([HealthCheckResult.unhealthy("gone")], name="jira", severity=Severity.OPTIONAL)
    runner = build(clock, scylla, jira, on_change=lambda status, reason, statuses: seen.append((status, statuses)))
    runner.start()
    await settle(jira)
    await spin()
    await runner.stop()
    assert seen == [(HEALTHY, {"scylla": HEALTHY, "jira": UNHEALTHY})]


async def test_a_stopped_runner_starts_again(clock):
    runner = HealthCheckRunner(service="zeus", clock=clock)
    ...
    await spin()
    await runner.stop()
    assert [check.name for check in runner.snapshot().checks] == ["jira"]

    second = ScriptedCheck([True], name="jira")
    subscription = runner.register(second)
    assert len(runner.snapshot().checks) == 1
    runner.start()
    assert (await subscription.wait_for(HEALTHY, timeout=2)).status is HEALTHY
    assert runner.snapshot().runner_up is True
    assert second.calls == 1
    await runner.stop()


async def test_a_registration_after_stop_probes_on_the_next_start(clock):
    runner = HealthCheckRunner(service="zeus", clock=clock)
    runner.start()
    await runner.stop()
    check = ScriptedCheck([True], name="jira")
    ...
    runner.start()
    await subscription.wait_for(HEALTHY, timeout=2)
    await runner.stop()


async def test_a_restarted_runner_announces_the_aggregate_again(clock):
    seen = []
    runner = HealthCheckRunner(
        service="zeus", clock=clock, on_change=lambda status, reason, statuses: seen.append(status)
    )
    for _ in range(2):
        check = ScriptedCheck([True], name="jira")
        runner.register(check)
        ...
        await spin()
        await runner.stop()
    assert seen == [HEALTHY, HEALTHY]


async def test_a_last_close_on_another_thread_cannot_strand_a_registration(clock):
    runner = HealthCheckRunner(service="zeus", clock=clock)
    runner.start()
    held = runner.register(ScriptedCheck([True], name="jira"))
    await spin()
    original = runner._slot
    closers = []

    def slot_then_close(check):
        slotted = original(check)
        closer = threading.Thread(target=held.close)
        closer.start()
        closer.join(0.05)
        closers.append(closer)
        return slotted

    runner._slot = slot_then_close
    fresh = runner.register(ScriptedCheck([True], name="jira"))
    runner._slot = original
    await asyncio.to_thread(closers[0].join)
    assert (await fresh.wait_for(HEALTHY, timeout=2)).status is HEALTHY
    assert runner.snapshot().checks[0].subscribers == 1
    await runner.stop()


async def test_a_shorter_interval_wakes_a_sleeping_probe_loop(clock):
    check = ScriptedCheck([True], name="jira", interval=3600)
    runner = build(clock)
    subscription = runner.register(check)
    runner.start()
    await subscription.wait_for(HEALTHY, timeout=2)
    await spin()
    runner.register(ScriptedCheck(name="jira", interval=0.01))
    await settle(check, calls=3)
    assert check.interval == 0.01
    await runner.stop()


async def test_a_first_unhealthy_result_is_logged(clock, caplog):
    check = ScriptedCheck([HealthCheckResult.unhealthy("token revoked")], name="github_api")
    runner = build(clock, check)
    with caplog.at_level(logging.INFO, logger="qatools_health"):
        runner.start()
        await settle(check)
        await spin()
    await runner.stop()
    assert "health check github_api PENDING -> UNHEALTHY: token revoked" in caplog.text
    assert [record.levelno for record in caplog.records if "PENDING" in record.getMessage()] == [logging.WARNING]


async def test_two_decorated_functions_register_side_by_side(clock):
    @healthcheck
    async def scylla():
        return True
    ...
    @healthcheck
    async def jenkins():
        return HealthCheckResult.degraded("queue 90")

    runner = build(clock)
    first, second = runner.register(scylla), runner.register(jenkins)
    runner.start()
    assert (await first.wait_for(HEALTHY, timeout=2)).status is HEALTHY
    assert (await second.wait_for(DEGRADED, timeout=2)).message == "queue 90"
    assert {check.name for check in runner.snapshot().checks} == {"scylla", "jenkins"}
    await runner.stop()


async def test_stop_never_announces_a_healthy_service_while_it_shuts_down(clock):
    seen = []
    check = ScriptedCheck(
        [HealthCheckResult.unhealthy("gone")], name="scylla", severity=Severity.CRITICAL, interval=0.01, delay=0.3
    )
    runner = build(clock, check, on_change=lambda status, reason, statuses: seen.append((status, statuses)))
    subscription = runner.register(ScriptedCheck(name="scylla"))
    runner.start()
    await subscription.wait_for(UNHEALTHY, timeout=2)
    await spin()
    await settle(check, calls=2)
    await runner.stop()
    await spin()
    assert seen == [(UNHEALTHY, {"scylla": UNHEALTHY})]
    assert runner.status is UNHEALTHY
    assert runner.snapshot().aggregate is UNHEALTHY
    assert runner.snapshot().runner_up is False


async def test_a_registration_after_stop_drops_the_stopped_checks(clock):
    runner = HealthCheckRunner(service="zeus", clock=clock)
    old = ScriptedCheck([True], name="jira")
    runner.register(old)
    runner.start()
    await settle(old)
    await runner.stop()
