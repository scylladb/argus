...
def handle_api_exception(exception: Exception):
    ...
    if issubclass(exception.__class__, APIException):
        LOGGER.info("[TraceId: %s] Endpoint %s responded with error %s: %s", trace_id, request.endpoint, exception.__class__.__name__, str(exception))
        LOGGER.info("[TraceId: %s] Headers\n%s", trace_id, request.headers)
        LOGGER.info("[TraceId: %s] Request Data Start\n%s\nRequest Data End", trace_id, request.json if request.is_json else request.get_data(as_text=True))
    else:
        LOGGER.error("[TraceId: %s] Exception in %s\n%s", trace_id, request.endpoint, "".join(format_exception(exception)))
