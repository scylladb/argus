...
def _create_view(api_client, name, items=None, settings='{"widgets": []}',
    ...
    return api_client.post("/api/v1/views/create", json=payload).json()
...
def test_index(api_client):
    res = api_client.get("/api/v1/views/").json()
    assert res["status"] == "ok"
    assert res["response"] == {"version": "v1"}


def test_create_view_success(api_client, view_name):
    res = _create_view(api_client, view_name, settings='{"widgets": [1]}',
                       description="desc-1", display_name="Disp")
    assert res["status"] == "ok"
    view_id = res["response"]["id"]

    fetched = api_client.get(f"/api/v1/views/get?viewId={view_id}").json()
...
def test_create_view_with_release_and_group_items(api_client, view_name, release, group, fake_test):
    res = _create_view(
        api_client,
        view_name,
    ...
    assert res["status"] == "ok"
    view_id = res["response"]["id"]
    resolved = api_client.get(f"/api/v1/views/{view_id}/resolve").json()["response"]
...
def test_create_view_duplicate_name_errors(api_client, view_name):
    _create_view(api_client, view_name)
    res = _create_view(api_client, view_name)
    assert res["status"] == "error"
    assert "already exists" in res["response"]["arguments"][0]


def test_get_view_missing_id_errors(api_client):
    res = api_client.get("/api/v1/views/get").json()
    assert res["status"] == "error"
    assert res["response"]["exception"] == "RequestValidationError"


def test_get_view_unknown_id_errors(api_client):
    missing_id = uuid.uuid4()
    res = api_client.get(f"/api/v1/views/get?viewId={missing_id}").json()
    assert res["status"] == "error"
    assert res["response"]["exception"] == "UserViewException"
    assert str(missing_id) in res["response"]["message"]


def test_resolve_view_tests_unknown_id_errors(api_client):
    missing_id = uuid.uuid4()
    res = api_client.get(f"/api/v1/views/{missing_id}/resolve/tests").json()
    assert res["status"] == "error"
    assert res["response"]["exception"] == "UserViewException"
    assert str(missing_id) in res["response"]["message"]
...
def test_all_views_no_filter(api_client, view_name):
    res = _create_view(api_client, view_name)
    view_id = res["response"]["id"]
    listing = api_client.get("/api/v1/views/all").json()["response"]
    assert any(v["id"] == view_id for v in listing)


def test_update_view_success(api_client, view_name, fake_test):
...
def test_delete_view_success(api_client, view_name):
    created = _create_view(api_client, view_name)
    view_id = created["response"]["id"]
    res = api_client.post("/api/v1/views/delete", json={"viewId": view_id}).json()
    assert res["status"] == "ok"
    assert res["response"] is True

    after = api_client.get(f"/api/v1/views/get?viewId={view_id}").json()
    assert after["status"] == "error"
    assert after["response"]["exception"] == "UserViewException"


def test_delete_view_unknown_id_errors(api_client):
    res = api_client.post("/api/v1/views/delete", json={"viewId": str(uuid.uuid4())}).json()
    assert res["status"] == "error"
    assert res["response"]["exception"] == "DocumentNotFound"


def test_search_no_query(api_client):
    res = api_client.get("/api/v1/views/search").json()
    assert res["status"] == "ok"
    assert res["response"] == {"hits": [], "total": 0}


def test_search_finds_release_by_name(api_client, release):
    res = api_client.get(f"/api/v1/views/search?query={release.name}").json()
    assert res["status"] == "ok"
...
def test_view_resolve_tests_returns_serialized_tests(api_client, view_name, fake_test):
    ...
    view_id = created["response"]["id"]
    res = api_client.get(f"/api/v1/views/{view_id}/resolve/tests").json()
    assert res["status"] == "ok"
    test_ids = {t["id"] for t in res["response"]}
    assert str(fake_test.id) in test_ids


def test_view_resolve_for_edit_includes_items(api_client, view_name, release, group):
    created = _create_view(
        api_client, view_name,
...
def test_view_versions_for_empty_view(api_client, view_name):
    created = _create_view(api_client, view_name)
    view_id = created["response"]["id"]
    res = api_client.get(f"/api/v1/views/{view_id}/versions").json()
    assert res["status"] == "ok"
    assert isinstance(res["response"], list)


def test_view_images_for_empty_view(api_client, view_name):
    created = _create_view(api_client, view_name)
    view_id = created["response"]["id"]
    res = api_client.get(f"/api/v1/views/{view_id}/images").json()
    assert res["status"] == "ok"
    assert isinstance(res["response"], list)


def test_view_pytest_results_empty(api_client, view_name):
    created = _create_view(api_client, view_name)
    view_id = created["response"]["id"]
    res = api_client.get(f"/api/v1/views/{view_id}/pytest/results").json()
    assert res["status"] == "ok"
    assert res["response"] == []


def test_view_stats_missing_view_id_errors(api_client):
    """`/views/stats` requires a viewId; absence raises ViewApiException."""
    res = api_client.get("/api/v1/views/stats")
    payload = res.json()
    assert payload["status"] == "error"
    assert payload["response"]["exception"] == "RequestValidationError"


def test_view_stats_for_empty_view_returns_ok(api_client, view_name):
    """Smoke-test `/views/stats` against an empty view; collector should return a stable shape."""
    created = _create_view(api_client, view_name)
    view_id = created["response"]["id"]
    res = api_client.get(f"/api/v1/views/stats?viewId={view_id}").json()
    assert res["status"] == "ok"
    assert isinstance(res["response"], dict)


def test_view_stats_with_widget_id_filter_returns_ok(api_client, view_name):
    ...
    created = _create_view(api_client, view_name, settings=settings)
    view_id = created["response"]["id"]
