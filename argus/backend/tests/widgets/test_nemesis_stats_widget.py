...
def test_nemesis_data_empty_view(api_client, empty_view):
    res = api_client.get(
...
def test_nemesis_data_unknown_view_errors(api_client):
    res = api_client.get(
...
def test_nemesis_data_missing_view_id_errors(api_client):
    res = api_client.get("/api/v1/views/widgets/nemesis_data").json()
    assert res["status"] == "error"


def test_nemesis_data_with_seeded_run(api_client, seeded_view_with_run, sct_run_with_nemesis):
