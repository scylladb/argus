...
def test_summary_versioned_runs_empty_view(api_client, empty_view):
    res = api_client.get(
...
def test_summary_versioned_runs_unknown_view_errors(api_client):
    res = api_client.get(
...
def test_summary_runs_results_empty_payload(api_client):
    res = api_client.post("/api/v1/views/widgets/summary/runs_results", json={}).json()
    assert res["status"] == "ok"
    assert res["response"] == {}


def test_summary_runs_results_unknown_test_returns_empty(api_client):
...
def test_summary_versioned_runs_with_seeded_view(api_client, seeded_view_with_run, sct_run):
...
def test_summary_runs_results_with_seeded_metric(api_client, generic_results_for_run):
