...
def test_pytest_view_returns_zero(api_client):
    res = api_client.get("/api/v1/views/widgets/pytest/view").json()
...
def test_pytest_view_results_empty(api_client):
    view_id = uuid.uuid4()
...
def test_pytest_release_results_empty(api_client):
...
def test_pytest_user_fields_for_unknown_returns_empty(api_client):
...
def test_pytest_user_fields_invalid_id_errors(api_client):
    res = api_client.get(
...
def test_pytest_view_results_with_seeded_row(api_client, seeded_pytest_row):
...
def test_pytest_user_fields_for_seeded_row(api_client, seeded_pytest_row):
