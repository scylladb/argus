...
async def submit_run(client_service: ClientService, testrun_service: TestRunService, fake_test: ArgusTest) -> SCTTestRun:
...
async def issue_added_events(run_id) -> list[dict]:
    events = await ArgusEvent.find(run_id=run_id).allow_filtering().all()
    return [json.loads(e.body) for e in events if e.kind == ArgusEventTypes.TestRunIssueAdded.value]


async def assert_refreshed_after(key: str, moment: datetime) -> None:
    """Scylla returns naive datetimes; treat the stored value as UTC before comparing."""
    value = (await RuntimeStore.get(key=key)).value
    if value.tzinfo is None:
        value = value.replace(tzinfo=UTC)
    assert value >= moment


async def issue_removed_events(run_id) -> list[dict]:
    events = await ArgusEvent.find(run_id=run_id).allow_filtering().all()
...
@pytest.fixture
async def run(client_service: ClientService, testrun_service: TestRunService, fake_test: ArgusTest) -> SCTTestRun:
    return await submit_run(client_service, testrun_service, fake_test)
...
async def test_submit_github_issue_link_for_existing_issue(run: SCTTestRun, mocked_issue_service: IssueService, logged_in_user: User):
    suffix = unique_suffix()
    url = f"https://github.com/scylladb/argus-{suffix}/issues/1"
    await GithubIssue(user_id=uuid4(), type="issues", owner="scylladb", repo=f"argus-{suffix}", number=1,
    ...
    issue_with_links = await mocked_issue_service.get("run_id", run.id, True)
    assert len(issue_with_links) == 1
    assert len(issue_with_links[0]["links"]) == 1

    events = await issue_added_events(run.id)
    assert len(events) == 1
    assert "attached" in events[0]["message"]
    assert events[0]["url"] == url


async def test_submit_jira_issue_link_for_existing_issue(run: SCTTestRun, mocked_issue_service: IssueService, logged_in_user: User):
    key = f"FROBNICATOR-{int(time.time_ns() % 10**9)}"
    url = f"{JIRA_SERVER}/browse/{key}"
    await JiraIssue(user_id=uuid4(), key=key, state="todo", summary="Example Issue", project="FROBNICATOR", permalink=url).save()
...
async def test_submit_unknown_github_issue_fetches_from_remote(run: SCTTestRun, mocked_issue_service: IssueService, logged_in_user: User):
    ...
    stored: GithubIssue = await GithubIssue.get(id=result["id"])
    ...
    link: IssueLink = await IssueLink.get(run_id=run.id, issue_id=stored.id)
...
async def test_submit_github_pull_request_is_stored_with_pull_type(run: SCTTestRun, mocked_issue_service: IssueService, logged_in_user: User):
    repo = f"argus-{unique_suffix()}"
...
async def test_submit_unknown_jira_issue_fetches_from_remote(run: SCTTestRun, mocked_issue_service: IssueService, logged_in_user: User):
    key = f"FROBNICATOR-{int(time.time_ns() % 10**9)}"
    url = f"{JIRA_SERVER}/browse/{key}"
    ...
    stored: JiraIssue = await JiraIssue.get(id=result["id"])
    assert stored.user_id == logged_in_user.id
...
async def test_submit_same_issue_twice_reuses_issue_and_adds_second_link(
    ...
    first_run = await submit_run(client_service, testrun_service, fake_test)
    second_run = await submit_run(client_service, testrun_service, fake_test)
    repo = f"argus-{unique_suffix()}"
    ...
    first = await mocked_issue_service.submit(issue_url=url, test_id=first_run.test_id, run_id=first_run.id, user=logged_in_user)
    second = await mocked_issue_service.submit(issue_url=url, test_id=second_run.test_id, run_id=second_run.id, user=logged_in_user)
    ...
    by_test = await mocked_issue_service.get("test_id", fake_test.id, aggregate_by_issue=True)
    ...
    assert "added" in (await issue_added_events(first_run.id))[0]["message"]
    assert "attached" in (await issue_added_events(second_run.id))[0]["message"]


async def test_submit_for_sct_event_records_event_id_on_link(run: SCTTestRun, mocked_issue_service: IssueService, logged_in_user: User):
    repo = f"argus-{unique_suffix()}"
    ...
    result = await mocked_issue_service.submit_for_sct_event(
        issue_url=url, test_id=run.test_id, event_id=str(event_id), run_id=run.id, user=logged_in_user)

    link: IssueLink = await IssueLink.get(run_id=run.id, issue_id=result["id"])
    assert link.event_id == event_id

    by_event = await mocked_issue_service.get("event_id", event_id)
...
async def test_submit_unknown_github_issue_with_remote_disabled_raises(run: SCTTestRun, issue_service: IssueService, logged_in_user: User):
    url = f"https://github.com/scylladb/argus-{unique_suffix()}/issues/1"

    with pytest.raises(Exception, match="Github Remote is disabled"):
        await issue_service.submit(issue_url=url, test_id=run.test_id, run_id=run.id, user=logged_in_user)

    assert await issue_service.get("run_id", run.id) == []
    assert await issue_added_events(run.id) == []


async def test_submit_malformed_github_url_raises_before_remote_call(run: SCTTestRun, mocked_issue_service: IssueService, logged_in_user: User):
    url = "https://github.com/scylladb/argus/commit/deadbeef"

    with pytest.raises(Exception, match="URL doesn't match Github schema"):
        await mocked_issue_service.submit(issue_url=url, test_id=run.test_id, run_id=run.id, user=logged_in_user)

    mocked_issue_service.gh.gh.get_repo.assert_not_called()
    assert await IssueLink.find(run_id=run.id).all() == []


async def test_submit_remote_github_failure_leaves_no_link(run: SCTTestRun, mocked_issue_service: IssueService, logged_in_user: User):
    url = f"https://github.com/scylladb/argus-{unique_suffix()}/issues/1"
    ...
    assert await IssueLink.find(run_id=run.id).all() == []
    assert await GithubIssue.find(url=url).all() == []
    assert await issue_added_events(run.id) == []


async def test_submit_remote_jira_failure_leaves_no_link(run: SCTTestRun, mocked_issue_service: IssueService, logged_in_user: User):
    key = f"FROBNICATOR-{int(time.time_ns() % 10**9)}"
    url = f"{JIRA_SERVER}/browse/{key}"
    ...
    assert await IssueLink.find(run_id=run.id).all() == []
    assert await JiraIssue.find(permalink=url).all() == []
...
async def test_refresh_stale_github_issues_updates_state_from_remote(run: SCTTestRun, mocked_issue_service: IssueService, logged_in_user: User):
    repo = f"argus-{unique_suffix()}"
    ...
    await mocked_issue_service.gh.refresh_stale_issues()
    ...
    refreshed: GithubIssue = await GithubIssue.get(id=submitted["id"])
    ...
    for_run = await mocked_issue_service.get("run_id", run.id)
    assert len(for_run) == 1
    assert for_run[0]["state"] == "closed"
    assert for_run[0]["subtype"] == "github"

    await assert_refreshed_after(GithubService.LAST_RAN_KEY, before_refresh)


async def test_refresh_stale_github_issues_ignores_unknown_and_case_variant_urls(run: SCTTestRun, mocked_issue_service: IssueService, logged_in_user: User):
    repo = f"Argus-{unique_suffix()}"
    ...
    assert (await GithubIssue.get(id=submitted["id"])).state == "closed"
    assert await GithubIssue.find(number=9999).allow_filtering().all() == []


async def test_refresh_stale_github_issues_survives_unreachable_repo(run: SCTTestRun, mocked_issue_service: IssueService, logged_in_user: User):
    repo = f"argus-{unique_suffix()}"
    ...
    assert (await GithubIssue.get(id=submitted["id"])).state == "open"
    assert (await RuntimeStore.get(key=GithubService.LAST_RAN_KEY)).value is not None


async def test_refresh_stale_jira_issues_updates_state_from_remote(run: SCTTestRun, mocked_issue_service: IssueService, logged_in_user: User):
    key = f"FROBNICATOR-{int(time.time_ns() % 10**9)}"
    url = f"{JIRA_SERVER}/browse/{key}"
    mocked_issue_service.jira.jira.issue.return_value = fake_remote_jira_issue(
    ...
    await assert_refreshed_after(JiraService.LAST_RAN_KEY, before_refresh)
...
async def test_refresh_only_touches_its_own_backend(run: SCTTestRun, mocked_issue_service: IssueService, logged_in_user: User):
    ...
    gh_submitted = await mocked_issue_service.submit(issue_url=gh_url, test_id=run.test_id, run_id=run.id, user=logged_in_user)
    jira_submitted = await mocked_issue_service.submit(issue_url=jira_url, test_id=run.test_id, run_id=run.id, user=logged_in_user)

    await mocked_issue_service.gh.refresh_stale_issues()
    mocked_issue_service.jira.jira.search_issues.assert_not_called()
    assert (await GithubIssue.get(id=gh_submitted["id"])).state == "closed"
    assert (await JiraIssue.get(id=jira_submitted["id"])).state == "to do"

    for_run = {i["subtype"]: i for i in await mocked_issue_service.get("run_id", run.id)}
    assert for_run["github"]["state"] == "closed"
    assert for_run["jira"]["state"] == "to do"

    await mocked_issue_service.jira.refresh_stale_issues()
    assert (await JiraIssue.get(id=jira_submitted["id"])).state == "done"
...
async def test_delete_github_issue_link_removes_orphaned_issue(run: SCTTestRun, mocked_issue_service: IssueService, logged_in_user: User):
    repo = f"argus-{unique_suffix()}"
    ...
    result = await mocked_issue_service.delete(issue_id=str(submitted["id"]), run_id=str(run.id), user=logged_in_user)

    assert result["deleted"] == submitted["id"]
    assert await mocked_issue_service.get("run_id", run.id) == []
    with pytest.raises(DocumentNotFound):
        await GithubIssue.get(id=submitted["id"])

    removed = await issue_removed_events(run.id)
    assert len(removed) == 1
    assert removed[0]["url"] == url


async def test_delete_jira_issue_link_falls_through_to_jira_backend(run: SCTTestRun, mocked_issue_service: IssueService, logged_in_user: User):
    key = f"FROBNICATOR-{int(time.time_ns() % 10**9)}"
    url = f"{JIRA_SERVER}/browse/{key}"
    mocked_issue_service.jira.jira.issue.return_value = fake_remote_jira_issue(key, summary="Deletable")
    submitted = await mocked_issue_service.submit(issue_url=url, test_id=run.test_id, run_id=run.id, user=logged_in_user)

    result = await mocked_issue_service.delete(issue_id=submitted["id"], run_id=run.id, user=logged_in_user)

    assert result["deleted"] == submitted["id"]
    assert await mocked_issue_service.get("run_id", run.id) == []
    with pytest.raises(DocumentNotFound):
        await JiraIssue.get(id=submitted["id"])
...
async def test_get_by_view_returns_each_link_once(api_client, run: SCTTestRun, mocked_issue_service: IssueService, logged_in_user: User):
    repo = f"argus-{unique_suffix()}"
    ...
    try:
        aggregated = await mocked_issue_service.get("view_id", view_id, aggregate_by_issue=True)
        flat = await mocked_issue_service.get("view_id", view_id)
    finally:
