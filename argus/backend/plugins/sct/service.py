...
class SCTService:
    ...
    @staticmethod
    def get_event_embeddings(run_id: str) -> list[dict]:
        """Get event embeddings data for a given test run

        Args:
            run_id: The test run ID to get embeddings for

        Returns:
            List of dictionaries containing event_index, severity and similars_set for each embedding
        """
        error_embeddings = ErrorEventEmbeddings.filter(run_id=run_id).all()
        critical_embeddings = CriticalEventEmbeddings.filter(run_id=run_id).all()
