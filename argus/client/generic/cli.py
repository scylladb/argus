...
@click.option("--release-name", help="Release name to filter plans by", default=None, required=False)
@click.option("--job-info-file", required=True, help="JSON file with trigger information (see detailed docs)")
def trigger_jobs(api_key: str, base_url: str, job_info_file: str, version: str, plan_id: str, release_name: str):
    client = ArgusGenericClient(auth_token=api_key, base_url=base_url)
    ...
    client.trigger_jobs({ "release": release_name, "version": version, "plan_id": plan_id, **payload })
