...
func buildAPIClientRaw(ctx context.Context, cfg *config.Config, extraOpts ...api.ClientOption) (*api.Client, error) {
	...
	if cfg.UseCf && !cfBypass {
		...
		case hasCFServiceTokenEnv():
			...
			log := LoggerFrom(ctx)
			log.Debug().Err(err).Msg("no Cloudflare Access token; using the CF Access service token")
		default:
...
// Non-interactive mode cannot, so it uses any unexpired token. Both modes warn
// when the token they return is about to expire.
func ensureCFToken(ctx context.Context, argusURL string, interactive bool) (string, error) {
	...
	var cfToken auth.CFToken
	if interactive {
		cfToken, err = argusSvc.GetOrFetchCFToken(ctx)
		if err != nil {
			return "", err
		}
	} else {
...
func runWithAuthRetry(cmd *cobra.Command, args []string, fn func(*cobra.Command, []string) error) error {
	...
	// Login() checks a stored PAT against Argus and replaces it when it is
	// rejected or expired, so only the stale session is dropped here.
	_ = keychain.Delete()
