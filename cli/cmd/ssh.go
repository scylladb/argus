...
var (
	...
	sshConnectCmd = &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			svc := services.NewSSHService(client)
			state, rotated, err := svc.EnsureKey(ctx, sshKeyPath, sshTTLSeconds, false)
			if err != nil {
				return err
			}

			if rotated {
				if _, err := svc.RegisterTunnel(ctx, state.PublicKey, sshTTLSeconds, sshTunnelID); err != nil {
					return err
				}
			}

			tunnelCfg, err := svc.GetTunnelConfig(ctx, sshTunnelID)
			if err != nil {
				return err
			}
