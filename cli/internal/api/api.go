...
// instead of sending req. It closes req.Body like [http.Client.Do] would, so a
// streaming producer is not left blocked. See [WithCFTokenUnavailable].
func (c *Client) preflight(req *http.Request) error {
	if c.cfTokenErr == nil {
		return nil
	}
	if req.Body != nil {
