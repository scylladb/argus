...
async def main():
    ...
    for group in await ArgusGroup.find(release_id=release.id).all():
        print(f"Saving group {group.id}")
        (dest / f"group_{group.id}.json").write_text(json.dumps(group, cls=ArgusJSONEncoder))

    print("getting test data")
    for test in await ArgusTest.find(release_id=release.id).all():
        ...
        for metadata in await ArgusGenericResultMetadata.find(test_id=test.id).allow_filtering().all():
        ...
        for graph_view in await ArgusGraphView.find(test_id=test.id).allow_filtering().all():
        ...
        for best_result in await ArgusBestResultData.find(test_id=test.id).allow_filtering().all():
    ...
    idx = 0
    for run in await SCTTestRun.find(release_id=release.id).all():
        idx += 1
        ...
        for result_data in await ArgusGenericResultData.find(run_id=run.id).allow_filtering().all():
        ...
        events = await SCTEvent.find(run_id=run.id).allow_filtering().all()
        if events:
        ...
        issue_links = await IssueLink.find(run_id=run.id).allow_filtering().all()
        if issue_links:
    ...
    all_issues = await GithubIssue.find().all()
    if all_issues:
